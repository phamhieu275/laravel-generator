...
class TableRelationshipsGenerator
{
    /** @var  string */
    public $tableName;
    ...
    public function generateRelationshipsFromTable()
    {
        $this->schema->getDatabasePlatform()->registerDoctrineTypeMapping('enum', 'string');
        $this->getEloquentRelationships();

        $relationFuncs = $this->generateFunctions();
        return $relationFuncs;
    }

    private function getEloquentRelationships()
    {
        foreach ($this->tables as $table) {
            ...
            foreach ($foreignKeys as $constraint) {
                $isOneToOne = $this->detectOneToOne($constraint, $primaryColumns);
    ...
    private function addOneToOneRules($table, $constraint)
    {
        ...
        /**
         * $table belongsTo $fkTable
         * $fkTable hasOne $table
         */
    ...
    private function addOneToManyRules($table, $constraint)
    {
        ...
        /**
         * $table belongs to $fkTable
         * $fkTable hasMany $table
         */
        $this->relationships[$table]['belongsTo'][] = [$fkTable, current($fields), current($references)];
    ...
    {
        $relationship = $this->relationships[$this->fromTableName];
        $functions = [];

        $functions += $this->generateHasOneFunctions($relationship['hasOne']);
        $functions += $this->generateBelongsToFunctions($relationship['belongsTo']);
        $functions += $this->generateHasManyFunctions($relationship['hasMany']);
        $functions += $this->generateBelongsToManyFunctions($relationship['belongsToMany']);
    ...
    private function generateBelongsToFunctions($rulesContainer)
    {
        ...
        foreach ($rulesContainer as $rules) {
            ...
            $function .= "\n\t\treturn \$this->hasOne('".$belongsToModel."', '$key1', '$key2');";
            $function .= "\n\t}";
    ...
    private function generateModelNameFromTableName($table)
    {
        ...
        $modelNameSpace = Config::get('generate.namespace_model', 'App\Models');
